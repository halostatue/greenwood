...
Contribution to `greenwood` is encouraged: bug reports, discussions, feature
...
`greenwood` is governed under the [Contributor Covenant Code of Conduct][cccoc].
...
`greenwood` contributions must be well understood by the submitter and that the
...
`greenwood` has adopted a variation of the Conventional Commits format for
...
Austin Ziegler created `greenwood`.
